...
class RequestHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        # 解析URL参数（示例格式：url=https://127.0.0.1:8080/capture?url=https://example.com）
...
def run(server_class=HTTPServer, handler_class=RequestHandler, port=8000):
    ...
    print(f"Starting server on 0.0.0.0:{port} (external access enabled, path=/hsil/sse)...")
    httpd.serve_forever()
